...
REM --- Load settings from config.env into the environment ---------------
for /f "usebackq eol=# tokens=1,* delims==" %%a in ("config.env") do set "%%a=%%b"

REM --- Validate the API key --------------------------------------------
if "%MT_EXPORT_API_KEY%"=="" (
  echo [X] No API key set. Run config.cmd, or paste your key into config.env.
  exit /b 1
)
echo %MT_EXPORT_API_KEY% | findstr /c:"tca_live_PASTE_YOUR_KEY_HERE" >nul
if not errorlevel 1 (
  echo [X] The API key in config.env is still the placeholder.
