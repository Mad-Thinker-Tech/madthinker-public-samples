...
- Photos: downloaded by default (opt-out). `MT_EXPORT_PHOTO_DIR` unset downloads signed photo URLs to `photos/`; set it to relocate, or to an empty string to skip. Download is best-effort, in the same iteration before the URLs expire. The mirror stores `photo_urls_expire_at` + local `photo_path`/`head_photo_path`, never the raw signed URLs.
- API contract and sync loop are fixed — see `README.md` and `client.py`/`sync.py` docstrings before changing behavior.
- The exported field list / table schema has ONE source of truth: `docs/TCA_Export_Reference.md`. The other docs link to it; do not re-enumerate fields elsewhere (that caused girth/scale/fin drift). Keep `ROW_FIELDS` in `store.py` in sync with it.
