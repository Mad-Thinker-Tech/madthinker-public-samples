...
- Package: `src/madthinker_export/` — `config` (env), `client` (HTTP + retries), `store` (SQLite), `sync` (loop), `__main__` (CLI).
...
- Keep it minimal: do not add tooling, frameworks, or scaffolding beyond the listed files.
