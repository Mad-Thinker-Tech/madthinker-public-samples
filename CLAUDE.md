...
- CLI commands: `python -m madthinker_export sync` (pull) and `show` (print the local mirror; reads only the DB path, no API key required).
- Onboarding wrappers (repo root): `config.env` (shared, non-secret settings — committed), `config.env.local` (holds the API key — git-ignored, not tracked; `config.cmd` recreates it on fresh clones), `config.cmd` (Windows setup: checks Python, builds the venv, installs, writes the key into `config.env.local`), `run.cmd` (Windows: loads config, runs sync), `showdata.cmd` (Windows: prints the catch reports in the local mirror). The three-step community flow is download → `config.cmd` → `run.cmd`, with `showdata.cmd` to view the data.
